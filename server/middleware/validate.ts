...
export const experienceValidation: ValidationChain[] = [
  body('nombre').trim().isLength({ min: 3, max: 200 }).escape().withMessage('Nombre inválido.'),
  body('descripcion').trim().isLength({ min: 10, max: 5000 }).escape().withMessage('Descripción inválida.'),
...
export const courseValidation: ValidationChain[] = [
  body('title').trim().isLength({ min: 3, max: 200 }).escape().withMessage('Título inválido.'),
  body('duration').trim().isLength({ min: 2, max: 100 }).escape().withMessage('Duración inválida.'),
  body('level').trim().isLength({ min: 2, max: 100 }).escape().withMessage('Nivel inválido.'),
  body('price').trim().isLength({ min: 1, max: 100 }).escape().withMessage('Precio inválido.'),
  body('priceDetail').optional({ values: 'falsy' }).trim().isLength({ max: 255 }).escape(),
  body('description').trim().isLength({ min: 10, max: 3000 }).escape().withMessage('Descripción inválida.'),
...
export const haciendaValidation: ValidationChain[] = [
  body('nombre').trim().isLength({ min: 3, max: 200 }).escape().withMessage('Nombre inválido.'),
  body('tipo').trim().isLength({ min: 2, max: 60 }).escape().withMessage('Tipo inválido.'),
  body('descripcion').trim().isLength({ min: 10, max: 5000 }).escape().withMessage('Descripción inválida.'),
  body('descripcion_corta').trim().isLength({ min: 5, max: 255 }).escape().withMessage('Descripción corta inválida.'),
  body('ubicacion').trim().isLength({ min: 2, max: 255 }).escape().withMessage('Ubicación inválida.'),
